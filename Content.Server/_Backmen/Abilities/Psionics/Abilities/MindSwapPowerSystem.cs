...
public sealed class MindSwapPowerSystem : EntitySystem
{
    ...
    public bool Swap(EntityUid performer, EntityUid target, bool end = false)
    {
        ...
        if (a)
        {
            if (performerMind!.UserId != null
                && _playerManager.TryGetSessionById(performerMind.UserId.Value, out var ps))
            {
                performerSession = ps;
            }
        ...
        if (b)
        {
            if (targetMind!.UserId != null
                && _playerManager.TryGetSessionById(targetMind.UserId.Value, out var ts))
            {
                targetSession = ts;
            }
